...
export default function HeroSection({ heroMovie, heroTrailerKey }: { heroMovie: any, heroTrailerKey: string | null }) {
  ...
  return (
    <section className="w-full max-w-7xl mx-auto mb-16 rounded-2xl overflow-hidden shadow-2xl bg-gray-900 flex flex-col md:relative md:aspect-video group">
      ...
      <div className="w-full aspect-video md:absolute md:inset-0 md:h-full">
        {heroTrailerKey ? (
          <iframe
            ...
            // The URL dynamically updates the mute parameter when the user clicks the button
      ...
      {heroTrailerKey && (
        <button
          ...
          className="absolute top-4 right-4 z-20 bg-black/40 hover:bg-black/80 text-white p-3 rounded-full backdrop-blur-md transition-all border border-white/10 shadow-xl"
